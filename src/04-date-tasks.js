...
function angleBetweenClockHands(date) {
  const convertTo12 = (h) => ((h < 12) ? h : h - 12);

  const hours = convertTo12(date.getUTCHours());
  const minutes = date.getUTCMinutes();
  if (!hours && !minutes) return 0;
  const angleMinutesDegr = 6 * minutes;
  const angleHoursDegr = 0.5 * (60 * hours + minutes);
  const result = angleHoursDegr - angleMinutesDegr;
  const angle = (Math.PI * result) / 180;
